...
/**
 * Gets a write reference from a given variable if the variable is reassigned only
 * once.
 *
    ...
    for (var i = 0; i < references.length; ++i) {
        ...
        if (reference.isWrite()) {
            if (retv && !(retv.init && reference.init)) {
                // This variable is reassigned two or more times.
                return null;
            }
            retv = reference;
        }
...
module.exports = function(context) {
    var APPLY_ONLY_TO_TOP_SCOPE = context.options[0] === 'top-scope';
    ...
    return {
        "Program:exit": function() {
            ...
            while (stack.length) {
                ...
                // Cloudability modification to add option to only check top level scope
                if (!APPLY_ONLY_TO_TOP_SCOPE || scope.block.type === "Program") {
